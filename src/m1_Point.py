...
class Point(object):
    ...
    def move_to(self, x, y):
        self.travel = self.travel + self.get_distance_from(Point(x, y))
        self.x = x
        self.y = y
        self.count = self.count + 1

    def move_by(self, dx, dy):
        self.travel = self.travel + self.get_distance_from(Point(self.x + dx, self.y + dy))
        self.x = self.x + dx
        self.y = self.y + dy
        self.count = self.count + 1
    ...
    def get_distance_from_start(self):
        P = Point(self.basex, self.basey)
        return self.get_distance_from(P)
    def get_distance_traveled(self):
        return self.travel

    def closer_to(self, P1, P2):
        distance1 = self.get_distance_from(P1)
        distance2 = self.get_distance_from(P2)
